...
function Standards({ standards }: { standards: StandardItem[] }) {
  return (
    <section className="rounded-3xl border border-border/70 bg-background/75 p-6 shadow-sm">
      <div className="flex items-center justify-between">
        <div>
          <h3 className="text-xl font-semibold">常用标准库</h3>
      ...
      <div className="mt-6 space-y-4">
        {standards.map((item, index) => (
          ...
          >
            <div className="flex flex-wrap items-start justify-between gap-3">
              <div>
                <div className="flex items-center gap-2">
                  <span className="rounded-full bg-secondary/15 px-2.5 py-0.5 text-sm font-medium text-secondary">
                    {item.type}
                  </span>
                  <p className="text-sm text-muted-foreground">更新 {item.updatedAt}</p>
                </div>
            ...
            <p className="mt-3 text-base text-muted-foreground">{item.tip}</p>
          </motion.div>
        ))}
      </div>
    </section>
...
function Pipeline({ stages }: { stages: PipelineStage[] }) {
  return (
    <section className="rounded-3xl border border-border/70 bg-background/75 p-6 shadow-sm">
      <div className="flex items-center justify-between">
        <div>
          <h3 className="text-xl font-semibold">备案流程追踪</h3>
          <p className="mt-1 text-base text-muted-foreground">
            查看各节点责任人、状态与预计完成时间。
          </p>
        </div>
          ...
          >
            ...
            <div className="flex-1">
              <div className="flex flex-wrap items-center justify-between gap-2">
                <div>
                  <p className="text-sm font-medium text-secondary/80">{stage.status}</p>
              ...
              <p className="mt-2 text-sm text-muted-foreground">预计完成：{stage.eta}</p>
            </div>
          </motion.div>
        ))}
      </div>
    ...
    <section className="space-y-4">
      ...
      <div className="overflow-hidden rounded-2xl border border-border/70 bg-background/80 shadow-sm">
        <table className="min-w-full divide-y divide-border/60 text-base">
          <thead className="bg-muted/50 text-left text-sm uppercase tracking-wide text-muted-foreground">
            <tr>
...
function Knowledge({ knowledge }: { knowledge: KnowledgeItem[] }) {
  return (
    <section className="rounded-3xl border border-border/70 bg-background/75 p-6 shadow-sm">
      <div className="flex items-center justify-between">
        <div>
          <h3 className="text-xl font-semibold">政策更新与知识库</h3>
          <p className="mt-1 text-base text-muted-foreground">
            实时关注标准更新、驳回案例与最佳实践。
          </p>
        </div>
