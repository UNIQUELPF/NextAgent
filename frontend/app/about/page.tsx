...
export default function AboutPage() {
  return (
    <div className="container space-y-12 text-base sm:text-lg">
      <header className="relative overflow-hidden rounded-3xl border border-border/70 bg-background/90 p-10 shadow-sm">
        ...
        <div className="space-y-6">
          <motion.span
            ...
            className="inline-flex items-center gap-2 rounded-full border border-secondary/40 bg-secondary/10 px-3 py-1 text-base font-semibold text-secondary"
          >
            企标邦 · 关于我们
          </motion.span>
          <motion.h1
      ...
      <section className="rounded-3xl border border-border/70 bg-background/80 p-8 shadow-sm">
        <div className="flex flex-col gap-6 lg:flex-row lg:items-start lg:justify-between">
          <div className="max-w-2xl space-y-4">
            <h2 className="text-3xl font-semibold">服务流程与保障</h2>
          ...
          <div className="grid w-full max-w-md gap-4 rounded-3xl border border-dashed border-secondary/50 bg-secondary/10 p-6">
            <h3 className="text-xl font-semibold text-secondary">联系我们</h3>
            <ul className="space-y-3 text-base text-secondary/90">
              {contact.map((item) => (
                <li key={item.label} className="rounded-2xl bg-white/60 p-3 shadow-sm dark:bg-slate-900/60">
                  ...
                  <p className="mt-1 text-lg text-secondary">{item.value}</p>
                </li>
              ))}
            </ul>
          </div>
